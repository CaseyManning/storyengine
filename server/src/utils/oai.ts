...
async function generate(prompt: string) {
	...
	const response = await client.responses.create({
		model: model,
		instructions:
			'You are a storytelling assistant. focus on creating compelling narratives given the context provided.',
		input: prompt,
	});
...
async function generate_structured(prompt: string, format: z.AnyZodObject) {
	...
	console.log(`generate request time: ${endTime.getTime() - startTime.getTime()}ms`);
